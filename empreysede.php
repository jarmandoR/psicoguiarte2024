...
$FB->titulo_azul1("Empresas",9,0,7);  
...
$FB->nuevoname("empresa", $condecion, "Agregar Empresa");
...
// SELECT `carp_id`, `carp_nombre` FROM `carpetasdocumentos`
		...
		// if($nivel_acceso==1 or  $nivel_acceso==20){
         ...
         $sql="SELECT `empre_id`, `empre_nombre` FROM `empresa` order by empre_nombre asc ";
...
// 	}
 ...
 //              $sql6="SELECT `sed_nombre`,`idsedes`FROM `sedes` WHERE `idsedes`='$rw3[1]'";
	...
	// 		$rw5=mysqli_fetch_row($DB->Consulta_ID);
		...
		echo"	
		<td><a  href='navempre.php?param2=".$rw1[0]."' ><img src='img/compania.png' width='40' height='34' href='navempre.php'>".$rw1[1]."<a/></td>
  ...
  //       }
		...
		if($nivel_acceso==1 or $nivel_acceso==18 or $nivel_acceso==20){
			$DB->edites($id_p, "empresa", 1, $condecion);
