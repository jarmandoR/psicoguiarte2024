...
$FB->nuevoname("carpetareglamento", $condecion, "Agregar carpeta");
...
// SELECT `carp_id`, `carp_nombre` FROM `carpetasdocumentos`
		...
		// if($nivel_acceso==1 or  $nivel_acceso==20){
         ...
         $sql="SELECT `carpregla_id`, `carpregla_nombre` FROM `carpetasregla` order by carpregla_nombre asc ";



		// }else{
		          
		//          $sql="SELECT * FROM `carpetasregla` inner join unicarpetas on unicar_idcarpeta=carp_id WHERE unicarp_idrol='$nivel_acceso' order by carpregla_nombre asc";
...
// 	}
 ...
 //              $sql6="SELECT `sed_nombre`,`idsedes`FROM `sedes` WHERE `idsedes`='$rw3[1]'";
	...
	// 		$rw5=mysqli_fetch_row($DB->Consulta_ID);
		...
		echo"	
		<td><a  href='navdocregla.php?param2=".$rw1[0]."' ><img src='images/carpeta.png' width='40' height='34' href='navdocregla.php'>".$rw1[1]."<a/></td>
  ...
  //       }
		...
		if($nivel_acceso==1 or $nivel_acceso==18 or $nivel_acceso==20){
			$DB->edites($id_p, "Carpetasreglamento", 1, $condecion);
