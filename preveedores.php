...
$FB->titulo_azul1("Proveedores",9,0,7);  
...
$FB->nuevoname("proveedor", $condecion, "Agregar Proveedor");
...
// SELECT `carp_id`, `carp_nombre` FROM `carpetasdocumentos`
		...
		// if($nivel_acceso==1 or  $nivel_acceso==20){
         ...
         $sql="SELECT `id_prove`, `nom_prove` FROM `proveedor` order by nom_prove asc ";



		// }else{
          
        //  $sql="SELECT * FROM `carpetasdocumentos` inner join unicarpetas on unicar_idcarpeta=carp_id WHERE unicarp_idrol='$nivel_acceso' and unicar_estado='1' order by carp_nombre asc";
...
// 	}
 ...
 //              $sql6="SELECT `sed_nombre`,`idsedes`FROM `sedes` WHERE `idsedes`='$rw3[1]'";
	...
	// 		$rw5=mysqli_fetch_row($DB->Consulta_ID);
		...
		echo"	
		<td><a  href='navprovee.php?param2=".$rw1[0]."' ><img src='img/repartidor.png' width='40' height='34' href='navprovee.php'>".$rw1[1]."<a/></td>
  ...
  //       }
		...
		if($nivel_acceso==1 or $nivel_acceso==18 or $nivel_acceso==20){
			$DB->edites($id_p, "proveedor", 1, $condecion);
