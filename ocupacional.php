...
$FB->abre_form("form1","terapiaOcupacion.php","post");
$FB->nuevo7("HistoriaParejas.php?condecion=datosocupacional&accion=1");
...
$FB->titulo_azul1("Consultante1",1,0,0); 
$FB->titulo_azul1("Consultante2 ",1,0,0); 
...
//    echo $sql="SELECT  `idPareja`, `hoj_fechaingreso`, `nombrePareja1`, `nombrePareja2` FROM `historiaPareja` inner join sedes on hoj_sede=idsedes   where idPareja>0  $conde1 $conde2 $cond5 $cond3 ORDER BY  nombrePareja1 asc LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
 ...
 $sql="SELECT `idPareja`, `nombrePareja1`, `nombrePareja2` FROM `historiaPareja` ORDER BY idPareja";
...
// 	//	echo "<td>"."Pendiente"."</td>";
		...
		$LT->llenaselect_ar22($rw1[22],$estadosac);
		 ...
		 $DB1->editar("terapiaOcupacion.php",$id_p, "terapiaOcupacion", 1,'datosparejas');
	}
